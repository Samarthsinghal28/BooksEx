...
var express = require("express"),
    ...
    async = require("async");
mongoose.connect(
    "mongodb+srv://Samarth:BP1ffeLkUs3Df1Jc@cluster0.9p2tkrn.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0",
...
app.get("/verify", function (req, res) {
    const mailOptions = {
        from: '"KitabBuddy Admin" <bookexexpert@gmail.com>',
        to: req.user.username,
        subject: "Email Confirmation",
        text:
            "Hello there, just a step away from creating your account. Here is your 6 digit pin. " +
            req.user.otp,
        html:
            "Hello there, just a step away from creating your account. Here is your 6 digit pin. <br><h2>" +
            req.user.otp +
            "</h2>",
    };
...
app.get("/secretkitab04848", isLoggedIn, async function (req, res) {
    ...
    Book.find({}, function (err, booksData) {
        ...
        Ebook.find({}, function (err, ebooksData) {
            if (err) console.log(err);
            Misc.find({}, function (err, miscData) {
                if (err) console.log(err);
                Feed.find({}, function (err, feedbackData) {
                    if (err) console.log(err);
...
// Admin accept routes
app.get("/secretkitab04848/:id/ab", function (req, res) {
...
app.get("/secretkitab04848/:id/aeb", function (req, res) {
...
app.get("/secretkitab04848/:id/am", function (req, res) {
    Misc.findByIdAndUpdate(
        req.params.id,
        { is_display: true },
        function (err, book) {
            if (err) console.log(err);
...
// Admin delete routes
app.delete("/secretkitab04848/:id/ab", function (req, res) {
    Book.findById(req.params.id, async function (err, book) {
        if (err) {
            console.log(err);
        }
...
app.delete("/secretkitab04848/:id/aeb", async function (req, res) {
...
app.delete("/secretkitab04848/:id/am", function (req, res) {
...
app.delete("/secretkitab04848/:id/feed", function (req, res) {
...
app.get(
    ...
    function (req, res) {
        ...
        if (
            ...
            req.user.college === undefined ||
            req.user.mobileno === undefined
        ) {
            res.redirect("/signup/detail");
        } else {
            res.redirect("/");
    ...
    Book.findById(req.params.id, function (err, book) {
        User.findById(req.user._id, function (err, buyer) {
            User.find({ username: book.uploader }, function (err, seller) {
                ...
                var seller_message =
                    "Hello there! Hope you are having a good day. Your product <strong>" +
                    book.title +
                    "</strong> just got a new customer. Its now your time to deal with the customer. <strong>Best of Luck!</strong> The user deatils are provided below: <br>Name: " +
                    req.user.firstname +
                    " " +
                    req.user.lastname +
                    " <br>Email ID: " +
                    ...
                    " <br>Phone Number: " +
                    req.user.mobileno +
                    " <h3>Please delete your book from the site once you have sold it.</h3>";
                ...
                var buyer_message =
                    "Hello there! Hope you are having a good day. Your just bought product <strong>" +
                    book.title +
                    "</strong>. Its now your time to deal with the Uploader. <strong>Best of Luck!</strong> The Uploader deatils are provided below: <br>Name: " +
                    seller[0].firstname +
                    " " +
                    seller[0].lastname +
                    " <br>Email ID: " +
            ...
            if (req.isAuthenticated()) {
                User.findById(req.user._id, function (err, user) {
                    ...
                    if (removelist.length > 0) {
                        var message =
                            "<strong>OOPS!</strong>, it seems like <strong>" +
                            removelist.length +
                            "</strong> of your Cart Items have been deleted by the owner and are NO longer available. Hence, this items have been removed from your cart.";
                        var new_notif = new notif({
                            content: message,
                        });
                        new_notif.save();
    ...
    UserData.populate("cart.item_id").exec(function (err, data) {
        if (err) console.log(err);
            ...
            UserData.populate("cart.item_id").exec(function (err, user) {
                ...
                for (var i = 0; i < len; i++) {
                    if (
                        user.cart[i].item_id._id.toString() ==
                        req.params.id2.toString()
                    ) {
...
app.get("/notif/:id/delete", function (req, res) {
    ...
    UserData.populate("notification").exec(function (err, data) {
        if (err) console.log(err);
            ...
            for (var i = 0; i < len; i++) {
                if (
                    data.notification[i]._id.toString() ==
